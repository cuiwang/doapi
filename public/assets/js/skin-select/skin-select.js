$(document).ready(function() { 
	...
	$("#skin-select #toggle").click(function() { 
		...
		if($(this).hasClass('active')) {
			$.cookie('skin-select_cookie', 'no_active'); // 存储 cookie
			...
			$.cookie('skin-select_cookie', 'active'); // 存储 cookie
...
//关闭侧栏
	if ($.cookie('skin-select_cookie') == 'active') {
		// show skin select for a second
		setTimeout(function(){ $("#skin-select #toggle").removeClass('active').trigger('click'); },10)
	} else  {
